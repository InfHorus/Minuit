...
function string.ToTable( str )
	...
	for i = 1, string_len (str) do--#str do
...
function string.Split(str, separator)
	...
	for i = 1, #str do
		if str:sub(i, i) == separator then
			table_insert(result, str:sub(currentIndex, i - 1))
			currentIndex = i + 1
		end
	end

	table_insert(result, str:sub(currentIndex))
...
function string.Explode(separator, str, withpattern)
  ...
  while current_pos <= #str do
    local start_pos, end_pos = string_find(str, separator, current_pos, not withpattern)
    if not start_pos then break end
    ret[#ret + 1] = str:sub(current_pos, start_pos - 1)
    current_pos = end_pos + 1
  end
...
function string.Split(str, separator)
  ...
  while currentIndex <= #str do
    local i = str:find(separator, currentIndex, true)
    if not i then break end
    table.insert(result, str:sub(currentIndex, i - 1))
    currentIndex = i + 1
  end

  table.insert(result, str:sub(currentIndex))
...
function string.NiceTime( seconds )
	...
	if ( seconds < 60 * 60 * 24 ) then
		local t = math_floor( seconds / (60 * 60) )
		return t .. pluralizeString( " hour", t )
	end

	if ( seconds < 60 * 60 * 24 * 7 ) then
		local t = math_floor( seconds / ( 60 * 60 * 24 ) )
		return t .. pluralizeString( " day", t )
	end

	if ( seconds < 60 * 60 * 24 * 365 ) then
		local t = math_floor( seconds / ( 60 * 60 * 24 * 7 ) )
		return t .. pluralizeString( " week", t )
	end

	local t = math_floor( seconds / ( 60 * 60 * 24 * 365 ) )
...
function string.Replace( str, tofind, toreplace )
	...
	local i = 1
	while true do
		local pos = string_find(str, tofind, startPos, true)
		if not pos then
			tbl[i] = string_sub(str, startPos)
			break
		end
		tbl[i] = string_sub(str, startPos, pos - 1)
		tbl[i + 1] = toreplace
		startPos = pos + string_len(tofind)
		i = i + 2
	end
...
function string.EndsWith( String, End )
	return End == "" or string_sub( String, -#End) == End
end
