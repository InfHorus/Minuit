...
function table.Add(dest, source)
  ...
  -- Iterate over the source table and insert the values into the destination table
...
function table.SortDesc( t )
	return table_sort( t, function( a, b ) return a > b end )
end
...
function table.SortByKey( t, desc )
	...
	for key, _ in pairs( t ) do table_insert( temp, key ) end
	if ( desc ) then
...
function table.SortByMember( Table, MemberName, bAsc )
	...
	table_sort( Table, function( a, b ) return TableMemberSort( a, b, MemberName, bAsc or false ) end )
...
local function toKeyValues( tbl )
	...
	for k,v in pairs( tbl ) do
		table_insert( result, { key = k, val = v } )
	end
...
function SortedPairs( pTable, Desc )
	...
	if ( Desc ) then
		table_sort( sortedTbl, function( a, b ) return a.key > b.key end )
	else
		table_sort( sortedTbl, function( a, b ) return a.key < b.key end )
	end
...
function SortedPairsByValue( pTable, Desc )
	...
	if ( Desc ) then
		table_sort( sortedTbl, function( a, b ) return a.val > b.val end )
	else
		table_sort( sortedTbl, function( a, b ) return a.val < b.val end )
	end
...
function RandomPairs( pTable, Desc )
	...
	if ( Desc ) then
		table_sort( sortedTbl, function(a,b) return a.rand > b.rand end )
	else
		table_sort( sortedTbl, function(a,b) return a.rand < b.rand end )
	end
